...
import (
	...
	"github.com/kanthub/wallet-sign/hsm"
	"github.com/kanthub/wallet-sign/leveldb"
...
func NewRpcServer(db *leveldb.Keys, config *RpcServerConfig) (*RpcServer, error) {
	...
	if config.HsmEnable {
		hsmClient, hsmError := hsm.NewHSMClient(context.Background(), config.KeyPath, config.KeyName)
		if hsmError != nil {
			log.Error("new hsm client fail", "err", hsmError)
		}
