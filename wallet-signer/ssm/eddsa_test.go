...
var eddsaSigner = &EdDSASigner{}

func TestCreateEdDSAKeyPair(t *testing.T) {
	privKey, pubKey, _ := eddsaSigner.CreateKeyPair()
...
func TestSignEdDSAMessage(t *testing.T) {
	...
	txMsg, _ := eddsaSigner.SignMessage(privKey, common.Hash{}.String())
...
func TestSignEdDSAMessageV2(t *testing.T) {
	...
	signature, err := eddsaSigner.SignMessage(fullPrivateKey, message)
...
func TestVerifyEdDSASign(t *testing.T) {
	...
	ok, err := eddsaSigner.VerifySignature(pubKey, common.Hash{}.String(), signature)
	fmt.Println("if sign ok=", ok)
	if !ok {
		fmt.Println("failure reason is", err)
	}
