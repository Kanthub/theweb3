...
func NewDB(ctx context.Context, dbConfig config.DBConfig) (*DB, error) {
	...
	db := &DB{
		...
		SupportToken:         NewSupportTokenDB(gorm),
		TokenPrice:           NewTokenPriceDB(gorm),
		SupportTokenExchange: NewSupportTokenExchangeDB(gorm),
	}
	return db, nil
}
...
func (db *DB) Transaction(fn func(db *DB) error) error {
	return db.gorm.Transaction(func(tx *gorm.DB) error {
		txDB := &DB{
			...
			SupportToken:         NewSupportTokenDB(tx),
			TokenPrice:           NewTokenPriceDB(tx),
			SupportTokenExchange: NewSupportTokenExchangeDB(tx),
		}
		return fn(txDB)
	})
}
