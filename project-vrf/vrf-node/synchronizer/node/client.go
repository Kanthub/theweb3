...
func (c *clnt) BlockHeaderByNumber(number *big.Int) (*types.Header, error) {
	ctxwt, cancel := context.WithTimeout(context.Background(), defaultRequestTimeout) // 100 s
	defer cancel()

	var header *types.Header
	err := c.rpc.CallContext(ctxwt, &header, "eth_getBlockByNumber", toBlockNumArg(number), false) // 区块结构体赋值到区块头，部分结构解码，是 Go 的 JSON 特性
	if err != nil {
...
func (c *clnt) BlockHeadersByRange(startHeight, endHeight *big.Int, chainId uint) ([]types.Header, error) {
	...
	if chainId != uint(global_const.PolygonChainId) {
		...
		numGroups := (int(count)-1)/groupSize + 1 // 301 / 100 + 1 =4
		wg.Add(numGroups)
		...
		for i := 0; i < int(count); i += groupSize {
			...
			go func(start, end int) { // 开四个进程每个拉取100区块
				defer wg.Done()
				for j := start; j <= end; j++ {
					ctxwt, cancel := context.WithTimeout(context.Background(), defaultRequestTimeout)
					defer cancel()
